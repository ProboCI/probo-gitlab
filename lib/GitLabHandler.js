...
let GitLabHandler = function(options) {
  ...
  // @todo: update this to work with gitlab instead of github
  this.server.get('/pull-request/:owner/:repo/:pullRequestNumber', function(req, res, next) {
    ...
    const gitlab = self.getGitLabApi({service_auth: {token: req.query.token}});
    // @todo: if repo doesn't work then we need the reaper to pass us the project id.
    gitlab.MergeRequests.show(req.params.repo, req.params.pullRequestNumber).then(function(result) {
      let output = {
        id: result.id,
        number: result.id,
        state: result.state === 'open' ? 'open' : 'closed',
        url: result.web_url,
        title: result.title,
