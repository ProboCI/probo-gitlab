...
var createWebhookHandler = require('gitlab-webhook-handler');
...
var GitLabHandler = function(options) {
  ...
  var log = bunyan.createLogger({name: 'gitlab-handler',
  ...
  this.server.get('/pull-request/:owner/:repo/:pullRequestNumber', function(req, res, next) {
    var gitlab = self.getGitLabApi({service_auth: {token: req.query.token}});
    var opts = {
      user: req.params.owner,
      repo: req.params.repo,
      number: req.params.pullRequestNumber,
    };
    gitlab.pullRequests.get(opts, function(error, pullRequest) {
      if (error) {
        res.json(500, error);
        return next();
      }
...
GitLabHandler.prototype.getGitLabApi = function(project) {
  var options = {
    // TODO: we are not supporting self hosted gitlab atm.
    url: this.options.gitLabUrl,
  };
  ...
  var gitlab = new GitLabApi(options);
  return gitlab;
};
...
GitLabHandler.prototype.pullRequestHandler = function(event, done) {
  // enqueue the event to be processed...
  var self = this;

  this.log.info('Gitlab Pull request ' + event.payload.pull_request.id + ' received');
...
GitLabHandler.prototype.buildStatusUpdateHandler = function(update, build, done) {
  ...
  // TODO: come back to this and fill in if possible.
  var task = this.postStatusToGitLab.bind(this, build.project, build.commit.ref, statusInfo);
 ...
 * @param {string} statusInfo - This should be the status message to post to GH. See http://docs.gitlab.com/ce/api/commits.html#post-the-build-status-to-a-commit
...
GitLabHandler.prototype.postStatusToGitLab = function(project, sha, statusInfo, done) {
  var self = this;
  var gitlab = self.getGitLabApi(project);

  statusInfo.user = project.owner;
  statusInfo.repo = project.repo;
  statusInfo.sha = sha;

  // TODO: the gitlab plugin doesn't support commit statuses
  // but the api does.  Should we fork it or forget it?
  gitlab.projects.statuses(statusInfo, function(error, body) {
    done(error, body);
  });
};
...
GitLabHandler.prototype.fetchProboYamlConfigFromGitLab = function(project, sha, done) {
  var gitlab = this.getGitlabApi(project);

  // TODO: support .?proviso.ya?ml and .?probo.ya?ml
  gitlab.projects.repository.showFile({projectId: project.repo, ref: sha, file_path: '.probo.yml'}, function(file) {
    if (!file) {
