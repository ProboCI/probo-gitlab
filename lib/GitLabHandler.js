...
console.log(11);
  this.server.get('/projects/:id/merge_requests', function(req, res, next) {
console.log(22);
...
//console.log(req);
console.log(333);
    //var test = gitlab.mergeRequests.show(6409287);
    var test = gitlab.mergeRequests.show(opts);
    //var test = gitlab.mergeRequests.all();
console.log(4);
    //console.log(test);


/**
    gitlab.mergeRequests.show(opts, function(error, pullRequest) {
console.log(3333);
      if (error) {
console.log(44);
        res.json(500, error);
...
console.log(66);
  ...
  //this.server.get('/pull-request/:owner/:repo/:pullRequestNumber', this.pullRequestController.bind(this));
...
GitLabHandler.prototype.postStatusToGitLab = function(project, sha, statusInfo, done) {
  ...
  gitlab.Projects.statuses(project.provider_id, sha, statusInfo.state, statusInfo).then(function(result) {
    done(null, result);
  }).catch((err) => {
    // This happens if we update the state with the same state and isn't a problem.
    if (err.error.message == 'Cannot transition status via :enqueue from :pending (Reason(s): Status cannot transition via "enqueue")') {
      return done();
    }
    return done(err);
  });
...
GitLabHandler.prototype.fetchYamlFromGitLab = function(path, sha, project, done) {
  ...
  gitlab.RepositoryFiles.show(project.provider_id, path, sha).then(function(file) {
    let content;
    let settings;
    if (file) {
      try {
...
GitLabHandler.prototype.pullRequestController = function(req, res, next) {
console.log('A');
/**
    var output = {
      id: 666,
...
console.log('AA');
  ...
  //var api = this.getGitLabApi({service_auth: {token: req.query.token}});
console.log('AAA');
  var opts = {
...
console.log('AAAA');
  ...
  api.MergeRequests.get(opts, function(error, pullRequest) {  
console.log('B');
    if (error) {
console.log('BB');
...
console.log('BBB');
    var output = {
      ...
      state: pullRequest.state === 'OPEN' ? 'openOOO' : 'closed!!!!',
      url: pullRequest.url,
      title: pullRequest.title,
