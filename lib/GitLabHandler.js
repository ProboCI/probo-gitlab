...
class GitLabHandler {
  ...
  constructor(options) {
    ...
    this.log = bunyan.createLogger({name: 'gitlab-handler',
    ...
    this.server = restify.createServer({log: this.log, name: 'Probo GLH'});
  ...
  _setupServer() {
    ...
    this.server.use(requestLogger({logger: this.log}));
   ...
   * @param {*} done - The callback to be called after the update is performed.
   */
  mergeRequestHandler(event, done) {
   ...
   */
  buildStatusUpdateHandler(update, build, done) {
    ...
    statusUpdateQueue.push(task, error => {
      if (error) {
        this.log.error({err: error, build_id: build.id}, 'An error occurred posting status to GitLab');
        return done(error, statusInfo);
      }

      this.log.info(statusInfo, 'Posted status to GitLab for', build.project.slug, build.commit.ref);
   ...
   */
  processMergeRequest(request, done) {
    this.log.info({type: request.type, id: request.id}, 'Processing merge request');

    this.api.findProjectByRepo(request, (error, project) => {
      if (error || !project) {
        return this.log.info({error}, `Project for gitlab project ${request.slug} not found`);
      }

      this.log.info({project: project}, 'Found project for PR');
      ...
      this.gitlab.fetchProboYamlConfig(project, request.sha, (error, config) => {
        ...
        if (error) {
          ...
          return this.buildStatusUpdateHandler(update, build, done);
        }

        this.log.info({config: config}, 'Probo Yaml Config file');

        this.submitBuild(request, project, config, done);
      });
    });
  }
   ...
   * @param {function} done - The callback to call when finished.
   */
  submitBuild(request, project, config, done) {
    let build = {
      commit: {
        ref: request.sha,
    ...
    this.api.submitBuild(build, project, (err, submittedBuild) => {
      if (err) {
        ...
        this.log.error({err: err, request: request, build: build, response: submittedBuild}, 'Problem submitting build');
