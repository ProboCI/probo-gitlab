...
const ProboEmailer = require('./ProboEmailer');
...
class Tokens {
  ...
  constructor(config, logger, apiLogger) {
    ...
    this.mail = new ProboEmailer(config, logger);
  }
  ...
  checkTokens(project) {
    ...
    if (statusCode != 200) {
      ...
      try {
        ...
        const email = ProboEmailer(this.config, this.logger);
        email.sendEmail({
          template: 'error',
          subject: 'ProboCI: GitLab Access Token Refresh Error',
          email_message: 'The access token for Organization ID:' + project.organizationId + ' could not be automatically refreshed.',
